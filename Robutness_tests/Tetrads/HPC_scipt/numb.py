...
def NumberOut(index):
    ...
    NumberNetworkoutout = pd.DataFrame(columns= ["Sender", "receiver", "pval"])
    Numb_sync = Median_Symbolise_3bins(np.random.normal(0, 0.35, length))
    Numb_d = {"Numb_sync": Numb_sync,
    ...
    NumberNetworkoutout = NumberNetworkoutout.append(CondTranEntNet(lista  = Numb_d,iterations = iteration_number))

    Numb_sync = Median_Symbolise_3bins(np.random.normal(0, 250, length))
    Numb_d = {"Composite_Crimes_sa_det": Composite_Crimes_sa_det,
         "Numb_sync": Numb_sync,
         "Pntweets_sa_det": Pntweets_sa_det, "Nntweets_sa_det": Nntweets_sa_det}
    NumberNetworkoutout = NumberNetworkoutout.append(CondTranEntNet(lista=Numb_d, iterations=iteration_number))

    Numb_sync = Median_Symbolise_3bins(np.random.normal(0, 28.22, length))
    Numb_d = {"Composite_Crimes_sa_det": Composite_Crimes_sa_det,
         "NewProQ_PolBrut_MainPapers_sa_det_lagged1": NewProQ_PolBrut_MainPapers_sa_det_lagged1,
         "Numb_sync": Numb_sync, "Nntweets_sa_det": Nntweets_sa_det}
    NumberNetworkoutout = NumberNetworkoutout.append(CondTranEntNet(lista=Numb_d, iterations=iteration_number))

    Numb_sync = Median_Symbolise_3bins(np.random.normal(0, 67.81, length))
    Numb_d = {"Composite_Crimes_sa_det": Composite_Crimes_sa_det,
         "NewProQ_PolBrut_MainPapers_sa_det_lagged1": NewProQ_PolBrut_MainPapers_sa_det_lagged1,
         "Pntweets_sa_det": Pntweets_sa_det, "Numb_sync": Numb_sync}
    NumberNetworkoutout = NumberNetworkoutout.append(CondTranEntNet(lista=Numb_d, iterations=iteration_number))
    NumberNetworkoutout.to_csv("Numb{}.csv".format(index))
...
def main():
    for i in range(Number_of_sync_series):
        Process(target=NumberOut, args=(i,)).start()
