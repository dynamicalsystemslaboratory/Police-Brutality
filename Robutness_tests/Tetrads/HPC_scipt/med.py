...
def MedianOut(index):
    ...
    MedianNetworkoutout = pd.DataFrame(columns=["Sender", "receiver", "pval"])
    Med_sync = Median_Symbolise_3bins(np.random.normal(0, 0.35, length))
    Med_d = {"Med_sync": Med_sync,
    ...
    MedianNetworkoutout = MedianNetworkoutout.append(CondTranEntNet(lista=Med_d, iterations=iteration_number))

    Med_sync = Median_Symbolise_3bins(np.random.normal(0, 250, length))
    Med_d = {"Composite_Crimes_sa_det": Composite_Crimes_sa_det,
             "Med_sync": Med_sync,
             "medianPtweets_sa_det": medianPtweets_sa_det, "medianntweets_sa_det": medianntweets_sa_det}
    MedianNetworkoutout = MedianNetworkoutout.append(CondTranEntNet(lista=Med_d, iterations=iteration_number))

    Med_sync = Median_Symbolise_3bins(np.random.normal(0, 0.06, length))
    Med_d = {"Composite_Crimes_sa_det": Composite_Crimes_sa_det,
             "NewProQ_PolBrut_MainPapers_sa_det_lagged1": NewProQ_PolBrut_MainPapers_sa_det_lagged1,
             "Med_sync": Med_sync, "medianntweets_sa_det": medianntweets_sa_det}
    MedianNetworkoutout = MedianNetworkoutout.append(CondTranEntNet(lista=Med_d, iterations=iteration_number))

    Med_sync = Median_Symbolise_3bins(np.random.normal(0, 0.06, length))
    Med_d = {"Composite_Crimes_sa_det": Composite_Crimes_sa_det,
             "NewProQ_PolBrut_MainPapers_sa_det_lagged1": NewProQ_PolBrut_MainPapers_sa_det_lagged1,
             "medianPtweets_sa_det": medianPtweets_sa_det, "Med_sync": Med_sync}
    MedianNetworkoutout = MedianNetworkoutout.append(CondTranEntNet(lista=Med_d, iterations=iteration_number))
    MedianNetworkoutout.to_csv("Med{}.csv".format(index))
    print(index)
...
def main():
    for i in range(Number_of_sync_series):
        Process(target=MedianOut, args=(i,)).start()
